...
class ReplayBuffer:
    ...
    def sample(self, batch_size, rng=default_rng()):
        ...
        return {
            ...
            # 'we_scores': samples['we_scores'],
        }

    def traj_sample(self, keep_seqs, T):
        '''
        adjacency: [T, N, N]
        num_edges: [T, 1]
        actions: [T, 1]
        ...
        B is the length of 'keep_seqs', T is the length of a single 
        element 'seq' in the 'keep_seqs' sequence
        '''        
        B = len(keep_seqs)
        adjacency    = np.zeros((B, T, self.num_variables, self.num_variables), dtype=np.uint8)
        num_edges    = np.zeros((B, T, 1), dtype=np.int_)
        actions      = np.zeros((B, T, 1), dtype=np.int_)
        delta_scores = np.zeros((B, T, 1), dtype=np.float_)
        mask         = np.zeros((B, T, self.num_variables, self.num_variables), dtype=np.uint8)       
        scores       = np.zeros((B, T, 1), dtype=np.float_)        
       
        for i, seq in enumerate(keep_seqs):
            samples = self._replay[seq]

            adjacency[i] = self.decode(samples['adjacency'])
            num_edges[i] = samples['num_edges']   
            actions[i] = samples['actions'] 
            delta_scores[i] = samples['delta_scores']
            mask[i] = self.decode(samples['mask'])
            scores[i] = samples['scores']
