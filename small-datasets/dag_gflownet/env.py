...
class GFlowNetDAGEnv(gym.vector.VectorEnv):
    ...
    def reset(self, L = 100):
        ...
        self.entropy = np.zeros((L, self.num_envs), dtype=np.float_)
        return deepcopy(self._state)

    def step(self, actions, entropy_index):
        ...
        se_scores = 0
        combine_scores = delta_scores
        # se_scores = self.dag_entropy2(dones, np.asarray(delta_scores))
        # self.entropy[entropy_index] = se_scores
        # se_scores1 = self.entropy[(entropy_index - 1) % 100]
        # se_scores2 = self.entropy[(entropy_index - 2) % 100]
        # combine_scores = self.combine_score(delta_scores, se_scores1, se_scores2)
        # se_scores = (se_scores + se_scores1 + se_scores2)/3
        
        return (deepcopy(self._state), combine_scores, se_scores, dones, {})
    ...
    def dag_entropy1(self, dones, delta_score):
        adj = self._state['adjacency']       # [M, N, N]
        ndgs = self._state['num_edges']      # 每个环境的边数
        M = adj.shape[0]                     # M 环境个数
        log2 = np.log(2.0)

        H = np.zeros_like(delta_score, dtype=np.float_)
        for m in range(M):
            if dones[m]:
                continue
            k = adj[m].sum(axis=0).astype(np.float_)    # [N]
            p = k / float(ndgs[m])                      # k/v
            # 熵：H = -∑plogp p=0的项贡献按极限视为0
            syb = p > 0
            H[m] = np.sum(p[syb] * (np.log(p[syb]) / log2))
        return H
    
    def dag_entropy2(self, dones, delta_score):
        adj = self._state['adjacency']       # [M, N, N]
        ndgs = self._state['num_edges']      # [M] 每个环境的边数
        M = adj.shape[0]                     # M 为环境个数
        N = adj.shape[1]
        log2 = np.log(2.0)
        ...
        for m in range(M):
            ...
            # 熵：H = -∑plogp p=0的项贡献按极限视为0
            syb = p > 0
            H[m] = np.sum(p[syb] * (np.log(p[syb]) / log2)) - np.log(1 / N)
        return H

    def dag_entropy3(self, dones, delta_score):
        adj = self._state['adjacency']       # [M, N, N]
        ndgs = self._state['num_edges']      # [M] 每个环境的边数
        M = adj.shape[0]                     # M为环境个数
        log2 = np.log(2.0)
        ...
        for m in range(M):
            ...
            # 熵：H = -∑plogp p=0的项贡献按极限视为0
            syb = p > 0
            H[m] = np.sum(3 * p[syb] * (np.log(p[syb]) / log2))
        return H
    
    def combine_score(self, bde_scores, se_scores1, se_scores2):
        l1 = 0.9
        l2 = 0.1
        return l1*bde_scores + l2*(se_scores1 - se_scores2)