...
def main(args):
    ...
    with trange(args.prefill + args.num_iterations, desc='Training') as pbar:
        for iteration in pbar:
            ...
            next_observations, delta_scores, we_scores, dones, _ = env.step(np.asarray(actions), entropy_index)
            indices = replay.add(
                observations,
                actions,
                logs['is_exploration'],
            ...
            entropy_index = (entropy_index + 1) % 100
            ...
            if iteration >= args.prefill:
                ...
                # keep_seqs, len_traj = traj.get_start(replay, batch_size=args.batch_size, rng=rng)
                # samples = replay.traj_sample(keep_seqs, len_traj)
                # params, state, logs = gflownet.step(params, state, samples)
    ...
    with open(args.output_folder /'records.jsonl', 'w') as f:
...
if __name__ == '__main__':
    ...
    optimization.add_argument('--num_iterations', type=int, default=20_000,
    ...
    # Bayesian network data small networks
    asia = subparsers.add_parser('asia')
    cancer = subparsers.add_parser('cancer')
    eqrthquake = subparsers.add_parser('earthquake')
    sachs = subparsers.add_parser('sachs')
    survey = subparsers.add_parser('survey')

    # Bayesian network data medium networks
    alarm = subparsers.add_parser('alarm')
    barley = subparsers.add_parser('barley')
    child = subparsers.add_parser('child')
    insurance = subparsers.add_parser('insurance')
    mildew = subparsers.add_parser('mildew')
    water = subparsers.add_parser('water')

    # Bayesian network data large networks
    hailfinder = subparsers.add_parser('hailfinder')
    hepar2 = subparsers.add_parser('hepar2')
    win95pts = subparsers.add_parser('win95pts')
