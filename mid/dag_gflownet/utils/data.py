...
def get_data(name, args, rng=default_rng()):
    midnetwork = ['alarm_interventional_bic', 
                  'barley_interventional_bic', 
                  'child_interventional_bic', 
                  'insurance_interventional_bic', 
                  'mildew_interventional_bic', 
    ...
    elif name in midnetwork:
        tmp = name.split('_')[0]
        graph = get_example_model(tmp)
        filename = get_data_txt(tmp, 1000)
        data = pd.read_csv(filename, delimiter=' ', dtype='category')
        score = 'bic'
