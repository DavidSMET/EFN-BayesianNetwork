...
def detailed_balance_loss(
    ...
    l3 = 0.05
    error = transformed_delta_scores + log_pB - log_pF - l3 * we_scores + log_pi_t[-1] - lax.stop_gradient(log_pi_tp1[-1])
...
def posterior_estimate(
    ...
    with trange(num_samples, disable=(not verbose), **kwargs) as pbar:
        while len(samples) < num_samples:
            ...
            observations, _, _, dones, _ = env.step(np.asarray(actions), entropy_index)
